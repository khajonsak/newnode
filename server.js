...
app.post('/loginrich', (req, res) => {
    ...
    if (username && password){
     ...
     if(results.length > 0){ 
         ...
         client.linkRichMenuToUser(userId, "richmenu-7baaa61984360c3a7e61639ef6653744");
        ...
        connection.query('SELECT * FROM user2 WHERE username = ? AND password = ?', [username, password], function(error, results, fields){
            if(results.length > 0){ 
                req.session.loggeedin = true;
                req.session.userId = userId;
                client.linkRichMenuToUser(userId, "richmenu-7313115713e65c97f5f96b8d1f7eb02f");
                ...
                connection.query('SELECT * FROM user3 WHERE username = ? AND password = ?', [username, password], function(error, results, fields){
                    if(results.length > 0){ 
                        req.session.loggeedin = true;
                        req.session.userId = userId;
                        client.linkRichMenuToUser(userId, "richmenu-9375c0c62c259c5b7566c47eadfdcfa5");
